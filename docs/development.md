...
for each 列:
    i19 = 0
    while i19 < 128 and 该列 i19 处是空或水:
        清空该处; i19++      # 从 y=0 向上抹掉空/水，直到首个实心
    for y = 127 down to i19:
        if y <= i19 + rand(6) - 1: 放基岩
        else: 按密度覆盖地表/填充
```

`i19` 是「从 y=0 数到首个实心方块的高度」。它常常落在有效高度域之外：

- 整列全空/全水时 `i19` 一直数到 128，越界；
- 柱体下方无致密填充时 `i19` 被推到接近 127。

两种情况下回填循环（`127..i19`）几乎或完全空转，基岩被铺在无效高度等于没铺，导致该列从地表贯穿到 y=0 全空，**直通虚空**。这就是 inf-20100625 版「海平面下几乎无方块、独石柱直抵虚空」的原因。inf-20100630-1340 起恢复为无此清洞段的逻辑，独石柱实心且直顶 128 高度上限。
